...
# =============================================
# 1. POSTGRESQL (SUPABASE) — DATA KEKAL SELAMANYA
# =============================================
@st.cache_resource
def get_db():
    return psycopg2.connect(
        st.secrets["SUPABASE_URL"].replace("https://", "postgres://") +
        f":5432/postgres?user=postgres&password={st.secrets['SUPABASE_KEY']}"
    )
...
def init_db():
    conn = get_db()
    cur = conn.cursor()
    cur.execute("CREATE TABLE IF NOT EXISTS documents (id SERIAL PRIMARY KEY, title TEXT, content TEXT, category TEXT DEFAULT 'Lain-lain', file_name TEXT, file_path TEXT, thumbnail_path TEXT, upload_date TIMESTAMP DEFAULT NOW())")
    cur.execute("CREATE EXTENSION IF NOT EXISTS pg_trgm")
    cur.execute("CREATE INDEX IF NOT EXISTS idx_fts ON documents USING GIN (title gin_trgm_ops, content gin_trgm_ops)")
    conn.commit()
    conn.close()
init_db()

# Backup penuh (download semua data + fail)
def create_backup():
    conn = get_db()
    cur = conn.cursor()
        ...
        for row in rows:
            if row[5]:  # file_path
                if os.path.exists(row[5]):
                    zf.write(row[5], f"uploads/{os.path.basename(row[5])}")
            if row[6]:  # thumbnail
                if os.path.exists(row[6]):
                    zf.write(row[6], f"thumbnails/{os.path.basename(row[6])}")
    mem.seek(0)
    return mem
