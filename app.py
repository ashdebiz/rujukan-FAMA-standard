...
# Password admin: admin → fama2025 | pengarah → fama123
ADMIN_CREDENTIALS = {
...
if page == "Halaman Utama":
    ...
    st.markdown("<p style='text-align:center;font-size:1.5rem;color:#2E7D32;'>Keluaran Hasil Pertanian Tempatan Malaysia</p>", unsafe_allow_html=True)
    ...
    with col1:
        cari = st.text_input("", placeholder="Cari tajuk standard...", key="cari_utama")
    with col2:
        kat = st.selectbox("", ["Semua"] + CATEGORIES, key="kat_utama")
...
# =============================================
# ADMIN PANEL — FULL POWER + CARIAN + EDIT FAIL PDF
# =============================================
else:
    ...
    with tab2:
        ...
        search = st.text_input("Cari tajuk / ID / kategori", placeholder="Contoh: durian, 25, Buah", key="cari_admin")
        st.markdown("</div>", unsafe_allow_html=True)
        ...
        if search:
            ...
            docs = [d for d in docs 
                    ...
                    or search_lower in str(d['id']) 
                    or search_lower in d['category'].lower()]
        ...
        for doc in docs:
            with st.expander(f"ID {doc['id']} • {doc['title']} • {doc['category']}"):
                ...
                with col2:
                    ...
                    if f"catidx_{doc['id']}" not in st.session_state:
                        st.session_state[f"catidx_{doc['id']}"] = CATEGORIES.index(doc['category']) if doc['category'] in CATEGORIES else 0

                    new_title = st.text_input("Tajuk", value=st.session_state[f"title_{doc['id']}"], key=f"ti_{doc['id']}")
                    new_cat = st.selectbox("Kategori", CATEGORIES, index=st.session_state[f"catidx_{doc['id']}"], key=f"ca_{doc['id']}")
                    ...
                    st.session_state[f"catidx_{doc['id']}"] = CATEGORIES.index(new_cat)
    ...
    # BACKUP & CHAT
    with tab3:
        st.markdown("### Backup Database & Fail")
        if st.button("Download Backup ZIP Sekarang", type="primary"):
            ...
            st.success("Backup siap! Simpan dengan selamat!")

        st.markdown("### Chat Pengguna")
        for m in reversed(get_chat_messages()):
            if m['is_admin']:
                st.success(f"Admin FAMA: {m['message']}")
            else:
