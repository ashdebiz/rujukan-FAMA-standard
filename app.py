...
def init_db():
    ...
    # Cipta table documents (dengan semua column)
    cursor.execute('''
    ...
    # AUTO UPGRADE: Tambah column `content` kalau tak wujud
    try:
        cursor.execute("SELECT content FROM documents LIMIT 1")
    except sqlite3.OperationalError:
        cursor.execute("ALTER TABLE documents ADD COLUMN content TEXT")
        st.success("Database dikemaskini secara automatik!")

    # Admin default
    cursor.execute("INSERT OR IGNORE INTO admins VALUES ('admin', ?)", 
                   (hashlib.sha256("fama2025".encode()).hexdigest(),))
    cursor.execute("INSERT OR IGNORE INTO admins VALUES ('pengarah', ?)", 
                   (hashlib.sha256("fama123".encode()).hexdigest(),))

    conn.commit()
    conn.close()

init_db()  # Jalankan sekali masa mula
...
def save_thumbnail_safely(uploaded_file, prefix="thumb"):
    ...
    try:
        ...
        if img.format not in ["JPEG", "JPG", "PNG", "WEBP"]:
            st.warning("Format gambar tidak disokong. Guna JPG/PNG sahaja.")
            return None
        ...
        st.error("Fail bukan gambar yang sah! Sila upload JPG/PNG.")
        return None
...
# =============================================
# ADMIN PANEL — 100% SELAMAT & AUTO-UPGRADE
# =============================================
else:
