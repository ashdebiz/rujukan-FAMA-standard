...
# =============================================
# DIRECT DARI QR CODE — SPESIFIK KOMODITI!
# =============================================
        ...
        if doc:
            ...
            <div class="direct-card">
                ...
                <p style="text-align:center; font-size:1.5rem; color:#2E7D32; margin:15px 0 0 0;">
                    Standard komoditi berikut telah dibuka secara langsung:
                </p>
            </div>
            """, unsafe_allow_html=True)
            ...
            with st.container():
                ...
                with c1:
                    img_path = doc['thumbnail_path'] if doc['thumbnail_path'] and os.path.exists(doc['thumbnail_path']) else "https://via.placeholder.com/400x600/4CAF50/white?text=FAMA"
...
# =============================================
# HALAMAN UTAMA — DENGAN STATISTIK CANTIK GILA!
# =============================================
if page == "Halaman Utama":
    ...
    <div style="background:linear-gradient(135deg, #00695c, #009688); border-radius:25px; padding:35px; color:white; margin:40px 0; box-shadow:0 20px 50px rgba(0,0,0,0.35);">
        ...
        <div style="display:grid; grid-template-columns:repeat(4,1fr); gap:20px; margin-top:40px;">
            {''.join(f'<div class="stat-box"><strong style="font-size:1.3rem;">{c}</strong><br><h2 style="margin:15px 0; font-size:3rem;">{cat_count.get(c,0)}</h2></div>' for c in CATEGORIES)}
        </div>
    </div>
    """, unsafe_allow_html=True)
...
# =============================================
# ADMIN PANEL — FULL POWER + BUTTON PADAM & CLEAR CHAT JALAN!
# =============================================
else:
    ...
    with tab2:
        ...
        for idx, d in enumerate(docs):
            with st.expander(f"ID {d['id']} • {d['title']} • {d['category']}"):
                ...
                with col1:
                    thumb_path = d['thumbnail_path'] if d['thumbnail_path'] and os.path.exists(d['thumbnail_path']) else "https://via.placeholder.com/300"
                    st.image(thumb_path, use_container_width=True)
                with col2:
                    new_title = st.text_input("Tajuk", value=d['title'], key=f"title_{d['id']}_{idx}")
                    new_cat = st.selectbox("Kategori", CATEGORIES, index=CATEGORIES.index(d['category']), key=f"cat_{d['id']}_{idx}")
                    new_pdf = st.file_uploader("Ganti PDF (pilihan)", type=["pdf"], key=f"pdf_{d['id']}_{idx}")
                    new_thumb = st.file_uploader("Ganti Thumbnail (pilihan)", type=["jpg","jpeg","png"], key=f"thumb_{d['id']}_{idx}")

                    if st.button("KEMASKINI", key=f"update_{d['id']}_{idx}", type="primary"):
                        updates = []
                        params = []
                        if new_title != d['title']:
                            updates.append("title=?"); params.append(new_title)
                        if new_cat != d['category']:
                            updates.append("category=?"); params.append(new_cat)
                        if new_pdf:
                            new_fp = f"uploads/{datetime.now().strftime('%Y%m%d_%H%M%S')}_{new_pdf.name}"
                            with open(new_fp, "wb") as f: f.write(new_pdf.getvalue())
                            updates.append("file_name=?, file_path=?"); params.extend([new_pdf.name, new_fp])
                            if os.path.exists(d['file_path']): os.remove(d['file_path'])
                        if new_thumb:
                            new_tp = save_thumbnail(new_thumb)
                            if new_tp:
                    ...
                    if st.button("PADAM STANDARD", key=del_key, type="secondary"):
                        if st.session_state.get(f"confirm_{del_key}", False):
                            ...
                            st.warning("Tekan sekali lagi untuk confirm padam standard ini!")
    ...
    with tab3:
        ...
        if messages:
            for msg in reversed(messages):
                if msg['is_admin']:
                    ...
                    balas = st.text_input("Balas mesej ini", key=f"reply_{msg['id']}")
        ...
        if st.button("PADAM SEMUA CHAT", type="primary"):
            if st.session_state.get("confirm_clear_all", False):
                ...
                st.warning("Tekan sekali lagi untuk confirm padam semua chat!")
